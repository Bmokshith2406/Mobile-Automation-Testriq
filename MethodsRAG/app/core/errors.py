...
class ProductionError(HTTPException):
    ...
    def _log_error(self):
        ...
        if self.status_code >= 500:
            logger.error(
                "server_error",
                extra={**log_payload, "error_message": self.error_message},
            )
        else:
            logger.warning(
                "client_error",
...
def unauthorized_error(message: str = "Authentication required") -> ProductionError:
...
def forbidden_error(message: str = "Forbidden") -> ProductionError:
